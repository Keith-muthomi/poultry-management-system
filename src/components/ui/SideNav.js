...
export class SideNav extends LitElement {
  ...
  navItem(path, label, icon) {
    ...
    return html`
      <a href=${path} data-link
        ...
        ${active 
          ? 'bg-md-primary/10 dark:bg-md-dark-primary/20 text-md-primary dark:text-md-dark-primary font-bold shadow-sm' 
  ...
  render() {
    return html`
      ...
      <aside class="fixed inset-y-0 left-0 z-50 flex flex-col h-screen bg-md-surface-container dark:bg-md-dark-surface-container border-r border-md-outline/10 dark:border-md-dark-outline/20 transition-all duration-300 transform 
        ${this.mobileOpen ? 'translate-x-0' : '-translate-x-full'} 
        md:translate-x-0 md:relative 
        ${this.collapsed ? 'md:w-20' : 'md:w-72'} w-72">

        <!-- Logo -->
        <div class="flex items-center gap-4 px-6 h-16 border-b border-md-outline/5 dark:border-md-dark-outline/10 mb-4">
        ...
        <nav class="flex flex-col flex-grow overflow-y-auto scrollbar-hide">
          ...
          <div class="my-4 border-t border-md-outline/5 dark:border-md-dark-outline/10 mx-6"></div>
        ...
        <!-- Footer / Collapse -->
        <div class="mt-auto p-3 border-t border-md-outline/5 dark:border-md-dark-outline/10">
           <button
            @click=${() => { this.collapsed = !this.collapsed; }}
            class="flex items-center gap-4 px-5 py-3 w-full rounded-md-full text-md-on-surface-variant dark:text-md-dark-on-surface-variant hover:bg-md-surface dark:hover:bg-md-dark-surface transition-colors"
          >
